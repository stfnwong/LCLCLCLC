...
// Expected traversals
const std::vector<int> exp_inorder_1 = {2, 1, 3};
const std::vector<int> exp_inorder_2 = {1, 3, 2};


TEST_CASE("TestTree, test_tree_depth", "[classic]")
{
    TreeNode* repr1_tree;

    repr1_tree = repr_to_tree(repr1);
    REQUIRE(nullptr != repr1_tree);
    REQUIRE(2 == tree_depth(repr1_tree));

    TreeNode* repr2_tree;

    repr2_tree = repr_to_tree(repr2);
    REQUIRE(nullptr != repr2_tree);
    REQUIRE(3 == tree_depth(repr2_tree));

    TreeNode* repr3_tree;

    repr3_tree = repr_to_tree(repr3);
    REQUIRE(nullptr != repr3_tree);
    REQUIRE(4 == tree_depth(repr3_tree));
}
...
TEST_CASE("TestTree, test_repr_to_tree", "[classic]")
{
    ...
    // the size here.
    REQUIRE(3 == tree_size(repr1_tree));
